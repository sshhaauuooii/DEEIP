...
class AnswerModule(nn.Module):
    def __init__(self,mode_arg):
        ...
        #self.LN=nn.LayerNorm(,elementwise_affine=True)
        self.clssifier=nn.Sequential(
            nn.Linear(768, 384).to(mode_arg.device),
            nn.GELU(),
            nn.Linear(384, 192).to(mode_arg.device),
            nn.GELU(),
            nn.Linear(192, 3).to(mode_arg.device),
        )

        self.skip=mode_arg.skip
    ...
    def get_crfmask(self,batch_input,batch_label,skip=True):
        ...
        for sentence,label in zip(batch_input,batch_label):
            ...
            if 1 not in label and 2 not in label and skip and (random.random()>self.skip):continue
