...
class InputModule(nn.Module):
    def __init__(self, mode_arg):
        ...
        self.bert=BertModel.from_pretrained(mode_arg.pre_model,cache_dir='../utils/bert',output_hidden_states=True).to(mode_arg.device)
        self.tokenizer=BertTokenizer.from_pretrained(mode_arg.pre_model,cache_dir='../utils/bert')
        ...
        self.bilinear=nn.Linear(1536,768)
    def forward(self, inputs):
        ...
        # if len([len(sentence)for batch in inputs for sentence in batch])==0:
        #     print()
        ...
        for sentence in b:
            ...
            bi_output.append(self.bilinear(output))
        return bi_output
